...
( function() {
	...
	CKEDITOR.plugins.add( 'image2', {
		...
		init: function( content ) {
			...
			if ( content.plugins.detectConflict( 'image2', [ 'easyimage' ] ) ) {
				return;
			}

			// Adapts configuration from orcontentiginal image plugin. Should be removed
			// when we'll rename image2 to image.
			var config = content.config,
				lang = content.lang.image2,
				image = widgetDef( content );
		...
		afterInit: function( content ) {
			// Integrate with align commands (justify plugin).
			var align = { left: 1, right: 1, center: 1, block: 1 },
				integrate = alignCommandIntegrator( content );

			for ( var value in align )content
				integrate( value );

			// Integrate with link commands (link plugin).
			linkCommandIntegrator( content );
		}
	} );
